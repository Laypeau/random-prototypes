...
public class Gen_Master : MonoBehaviour
{
	...
	void Awake()
	{
		if (focus == null || prefab_Tile == null) throw new UnityException("Focus/tile object not set");
	...
	void Init()
	{
		...
		for (int z = -radius; z < radius; z++)
		{
			for (int x = -radius; x < radius; x++)
			{
				...
				CreateTile(activeTiles[i].ToString(), new Vector3(focus.position.x + (tileSize * activeTiles[i].x), 0f, focus.position.z + (tileSize * activeTiles[i].y)));
				i++;
			}
		}
	}
